...
_CALCULATION_BY_METRIC_ID = {
    ...
    "hr_recovery_pauses": "Detected pause count from HR recovery analysis; pauses are >= Config.Metrics.MIN_PAUSE_SEC with velocity < Config.Thresholds.VEL_STOP. The v5 read model currently does not persist this count, so MCP payloads return null.",
    "hr_recovery_total_rest_sec": "Total seconds across HR recovery pauses; pause detection matches hr_recovery_pauses. The v5 read model currently does not persist this total, so MCP payloads return null.",
    ...
    "cardiac_cost_adjusted": "Currently materialized as the same value as cardiac_cost in the v5 read model; legacy analytics used cardiac_cost - Config.Efficiency.CC_ELEV_COEFF * elevation_m_per_km.",
    ...
    "cardiac_drift_severity": "Severity label emitted by the Jenks-based cardiac drift algorithm for the activity.",
    "cardiac_drift_quality": "Quality label emitted by the cardiac drift algorithm; the v5 read model currently does not persist it, so MCP payloads return null.",
    "cardiac_drift_significant": "Current comparison payload maps materialized cardiac_drift_pct to 1 when drift_pct >= 5.0, otherwise 0.",
    ...
    "z5_seconds": "Count of stream heartrate samples at or above the configured Z5 lower bound.",
    "hr_anomaly_count": "Count of consecutive stream heartrate jumps greater than 30 bpm.",
    "daily_trimp": "Daily effective_trimp from daily_load_facts, built from per-activity TRIMP and zero-filled rest or missing-HR days.",
    ...
    "by_sport_trimp": "Distribution of TRIMP grouped by sport over a period; currently not materialized for v5 get_fitness_state.",
    "by_sport_distance_km": "Distribution of mirrored distance_m grouped by sport over a period and divided by 1000; currently not materialized for v5 get_fitness_state.",
    "by_sport_time_min": "Distribution of mirrored moving_time_s grouped by sport over a period and divided by 60; currently not materialized for v5 get_fitness_state.",
    "by_sport_elevation_m": "Distribution of mirrored elevation_gain_m grouped by sport over a period; currently not materialized for v5 get_fitness_state.",
    ...
    "form_zone": "Categorizes form as tired when form < -5, normal when -5 <= form < 10, and fresh when form >= 10.",
    "banister_history": "Daily time series of Banister fitness, fatigue, form, and TRIMP from the materialized training_model_daily facts.",
    "acwr": "Acute:chronic workload ratio stored as fatigue / fitness when fitness > 0.",
    "acwr_zone": "ACWR category derived from acwr thresholds in report logic; currently not materialized as a v5 read-model column.",
    "atl": "Alias for Banister fatigue in the v5 read model.",
    "ctl": "Alias for Banister fitness in the v5 read model.",
    "acwr_history": "Daily time series of acwr values from training_model_daily facts.",
    ...
    "rolling_median_cc": "Reserved 90-day rolling median cardiac_cost; the v5 materializer currently writes null.",
    "rolling_median_cc_adj": "Reserved 90-day rolling median adjusted_cardiac_cost; the v5 materializer currently writes null.",
    "rolling_median_epkm": "Reserved rolling median elevation gain per kilometer; currently not materialized in v5 read-model payloads.",
    "volume_7d": "Activity count over the 7-day rolling window.",
    "volume_28d": "Activity count over the 28-day rolling window.",
    "load_trend_pct": "Trend percent over rolling effective_trimp; currently backed by the 28-day effective_trimp fact in get_fitness_state.",
    "efficiency_trend_pct": "Reserved trend of per-sport efficiency metrics such as cardiac cost; currently skipped for period comparison.",
    "run_90d_median_cc_trend_pct": "Reserved trend of 90-day running median cardiac cost; currently skipped for period comparison.",
    "activity_streak_days": "Reserved count of consecutive recent active days; current get_fitness_state returns null.",
    "rest_streak_days": "Reserved count of consecutive recent rest days; current get_fitness_state returns null.",
    "last_hike_days_ago": "Reserved days since the latest Hike activity; current get_fitness_state returns null.",
    "progressive_load_bonus": "Reserved progressive-overload scalar from 21-day quality trends; current MCP payloads return null.",
    "progressive_cc_trends": "Reserved per-sport cardiac-cost trend bundle from progressive signal analysis; current MCP payloads return null.",
