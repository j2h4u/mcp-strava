...
def daily_report_from_connection(conn, now_local=None):
    """Compute the full daily training report from an existing SQLite connection.

    Preserves daily_report() calculations while allowing application services and
    tests to inject a fixture connection and clock.
    """
...
def daily_report():
    """Compute the full daily training report from the configured local mirror."""
    with DbConn() as conn:
        return daily_report_from_connection(conn)
