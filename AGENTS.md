...
- `Justfile` defines the local command surface; `just check` runs static quality only, `just unit` runs pytest only, `just runtime` runs Docker build/start plus MCP smoke only, `just verify` runs the full local gate, and `just typecheck-tests` is an opt-in debt gate not required for completion.
