...
def _create_read_model_dirty_fixture(db_path: Path) -> None:
    from mcp_strava.adapters.sqlite.migrations import run_migrations
    ...
    with SQLiteRepository.from_path(db_path) as repo:
        assert repo.update_activity_source_state_and_enqueue_dirty(
            1,
            reason="cli_test",
            metric_version=1,
            queued_at="2026-05-21T12:00:00",
        )
        repo.conn.commit()


def test_admin_read_model_materialize_dry_run_json_output(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    _create_read_model_dirty_fixture(fixture)
    materialize_calls: list[object] = []

    monkeypatch.setattr(cli, "materialize_read_model", lambda *args, **kwargs: materialize_calls.append((args, kwargs)), raising=False)
    monkeypatch.setattr(
        sys,
        "argv",
        [
    ...
    assert payload["dirty_count_before"] == 1
    assert payload["dirty_count_after"] == 1
    assert payload["facts_written"] == 0
    assert payload["oldest_dirty_day"] == "2026-01-01"
    assert payload["run_id"] is None
    assert materialize_calls == []


def test_admin_read_model_materialize_json_runs_materializer_with_limit_and_version(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    def fake_materialize(repo, *, metric_version, limit=None, now=None, run_id=None, renew_lease=None):
        del now, renew_lease
        rows = repo.dirty_activity_rows(metric_version)
        calls.append({"metric_version": metric_version, "limit": limit, "run_id": run_id, "dirty": len(rows)})
        recorded_run_id = repo.record_read_model_refresh_run(
            {
                "started_at": "2026-05-21T12:00:00",
                "finished_at": "2026-05-21T12:00:00",
                "status": "ok",
                "metric_version": metric_version,
                "trigger_reason": "admin_read_model_materialize",
                "activities_considered": len(rows),
                "activities_materialized": len(rows),
                "dirty_rows_claimed": len(rows),
                "dirty_rows_cleared": len(rows),
                "attempt_count": 1,
                "last_error": None,
            }
        )
        cleared = repo.clear_dirty_activity_rows(rows)
        repo.conn.commit()
        return {
            "status": "ok",
            "activities_materialized": len(rows),
    ...
    monkeypatch.setattr(cli, "materialize_read_model", fake_materialize, raising=False)
    monkeypatch.setattr(
        sys,
        "argv",
        [
    ...
    assert calls == [{"metric_version": 1, "limit": 1, "run_id": None, "dirty": 1}]
    assert payload["status"] == "ok"
    assert payload["metric_version"] == 1
    assert payload["dirty_count_before"] == 1
    assert payload["dirty_count_after"] == 0
