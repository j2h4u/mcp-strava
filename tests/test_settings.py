...
def test_load_settings_defaults() -> None:
    ...
    assert settings.http.bearer_token is None
...
def test_load_settings_environment_overrides() -> None:
    env = {
        ...
        "MCP_STRAVA_HTTP_BEARER_TOKEN": "shared-secret",
    ...
    assert settings.http.bearer_token == "shared-secret"
...
def test_load_settings_env_file_compatibility(tmp_path: Path) -> None:
    ...
    env_file.write_text(
        "\n".join(  # noqa: FLY002 — multi-line list reads far better than one giant literal
            [
                ...
                "MCP_STRAVA_HTTP_BEARER_TOKEN=file-secret",
    ...
    assert settings.http.allow_container_bind is True
    assert settings.http.bearer_token == "file-secret"
    assert settings.http.allowed_hosts == ("127.0.0.1", "localhost", "mcp-strava")
...
def test_http_bearer_token_is_omitted_from_settings_repr() -> None:
    settings = load_settings(
        environ={"MCP_STRAVA_HTTP_BEARER_TOKEN": "secret-literal"},
        project_root=Path("/tmp/project"),
    )

    assert settings.http.bearer_token == "secret-literal"
