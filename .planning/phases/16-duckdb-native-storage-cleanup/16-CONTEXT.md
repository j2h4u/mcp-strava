...
- `schema_tables.py:135,158,183` (daily_load_facts / training_model_daily / rolling_period_facts). Stored as a JSON-array string, aggregated via DuckDB `list()` (`aggregate_queries.py:294,377,462`) then **double-decoded** in Python (`aggregate_rows.py:169-181` json.loads each element). Convert column to native `VARCHAR[]` so `flatten`/`list_distinct` work in SQL and the Python double-`json.loads` disappears. Touches schema + aggregate SQL + write-path serialization + decode. Medium effort.
